...
local shot = function(mode)
	...
	local cmd = 'maim --hidecursor --nokeyboard ' .. select .. ' ' .. file_loc
	awful.spawn.easy_async(cmd, function()
		local clip_cmd = 'xclip -selection clipboard -t image/png -i ' .. file_loc
		...
		open_image:connect_signal('invoked', function()
			awful.spawn(apps.default.image_viewer .. ' ' .. file_loc, false)
		end)

		open_folder:connect_signal('invoked', function()
			awful.spawn(apps.default.file_manager .. ' ' .. home, false)
		end)
